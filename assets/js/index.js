...
function showNavSearchResults(q, container) {
    ...
    container.innerHTML = matches.slice(0, 8).map(p => {
        ...
        const link = p.label ? `product.html?label=${p.label}` : `product.html?id=${p.id}`;
        return `
            <a href="${link}">
...
function initStaticCategories() {
    ...
    categories.forEach(cat => {
        html += `
            <div class="category-card" onclick="window.location.href='index.html?category=${encodeURIComponent(cat.name)}'">
