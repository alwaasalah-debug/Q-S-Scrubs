...
function switchTab(tabName) {
    ...
    btnCategories.classList.replace('btn', 'btn-outline');
    ...
    if (tabName === 'products') {
        ...
        btnCategories.classList.replace('btn-outline', 'btn');
    ...
    orders.forEach(order => {
        ...
        if (Array.isArray(order.items)) {
            order.items.forEach(item => {
                itemsHtml += `<li style="margin-bottom: 4px;">- ${item.name} <span style="color:#666">(${item.size}, ${item.color})</span> x${item.quantity}</li>`;
            });
        }
        itemsHtml += '</ul>';
...
let currentCategoryImageUrl = null; // Store current image URL when editing
...
// Reset form to add mode
function resetCatFormToAddMode() {
    document.getElementById('category-id').value = '';
    document.getElementById('category-form-title').textContent = 'إضافة قسم جديد';
    document.getElementById('submit-cat-btn').textContent = 'حفظ القسم';
    document.getElementById('add-category-form').reset();
    document.getElementById('cat-image-preview-container').innerHTML = '';
...
async function handleAddCategory(e) {
    ...
    try {
        ...
        if (!name || !label) {
            alert('يرجى إدخال اسم القسم والـ Label.');
            return;
        }

        let imageUrl = currentCategoryImageUrl; // keep existing if no new upload

        // Upload new image if selected
        if (imageInput.files && imageInput.files.length > 0) {
            const file = imageInput.files[0];
            if (file.size > 1 * 1024 * 1024) {
                alert('حجم الصورة أكبر من 1 ميجابايت.');
                return;
            }
            ...
            if (uploadedUrl) {
                ...
                throw new Error("فشل رفع صورة القسم");
            }
        }
        ...
        if (id) {
            // Update mode
            const { error } = await updateCategoryInDB(id, categoryData);
            if (error) throw new Error(error);
            alert('تم تحديث القسم بنجاح!');
        } else {
            // Add mode
            if (!imageUrl) {
                alert('صورة القسم مطلوبة عند الإضافة.');
                return;
            }
            const { error } = await addCategoryToDB(categoryData);
            if (error) throw new Error(error);
            alert('تم إضافة القسم بنجاح!');
        }

        resetCatFormToAddMode();
        renderCategoriesTable();
...
async function renderCategoriesTable() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">جاري تحميل الأقسام...</td></tr>';

    const categories = await getCategories();
    tbody.innerHTML = '';

    if (categories.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">لا توجد أقسام حالياً</td></tr>';
        return;
    }
    ...
    categories.forEach(cat => {
        const row = `
            <tr>
                <td><img src="${cat.image_url || ''}" class="admin-img-thumb" style="object-fit: contain; background: #f0f0f0;" alt="img"></td>
                <td>${cat.name}</td>
                <td style="direction: ltr; text-align: right;">${cat.label}</td>
                <td>${cat.sort_order}</td>
                <td>
                    <button class="btn btn-sm" onclick='handleEditCategory(${JSON.stringify(cat)})' style="transform: scale(0.8); background: #4CAF50;">تعديل</button>
                    <button class="btn btn-sm" onclick="handleDeleteCategory('${cat.id}')" style="background:red; transform: scale(0.8)">حذف</button>
                </td>
            </tr>
        `;
        tbody.insertAdjacentHTML('beforeend', row);
...
function handleEditCategory(cat) {
    ...
    currentCategoryImageUrl = cat.image_url;

    document.getElementById('category-form-title').textContent = 'تعديل القسم';
    document.getElementById('submit-cat-btn').textContent = 'تحديث القسم';
    ...
    document.querySelector('#categories-view').scrollIntoView({ behavior: 'smooth' });
}
